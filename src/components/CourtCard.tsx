...
interface CourtCardProps {
  match: Match;
  onWinner?: (winner: "teamA" | "teamB") => void;
}
  ...
  return (
    <div className={`court-card${match.winner ? " has-winner" : ""}`}>
      ...
      <div className="court-matchup">
        <div className={`team team-a${match.winner === "teamA" ? " winner" : match.winner === "teamB" ? " loser" : ""}`}>
          {match.teamA.map((p) => p.name).join(" & ")}
        </div>
        <div className="vs">vs</div>
        <div className={`team team-b${match.winner === "teamB" ? " winner" : match.winner === "teamA" ? " loser" : ""}`}>
          {match.teamB.map((p) => p.name).join(" & ")}
        </div>
      </div>
